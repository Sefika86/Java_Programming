...
public class IPhone {
    ...
    public boolean equals(Object obj) {
        if (! (obj instanceof IPhone) ) {  //if the given object is an iphone
            System.err.println("Invalid object, Object must be Iphone");
            System.exit(1);
        }

            if (model.equals(((IPhone) obj).model)) {  // if the iphone model is equal to given iphone's model ,polymorphism is a MUST otherwise compile error
                if (color.equals(((IPhone) obj).color)) { //if the colors are same too
                    return true;
                }
            }
